# Handoff Context — 2026-07-23 (🎉 PROJECT HOÀN THÀNH — cả 7 phases xong. App live: applyist.vercel.app)

> **Update 2026-07-23 — 7B-3 XONG → PHASE 7 ĐÓNG → TOÀN BỘ PLAN 7 PHASES HOÀN THÀNH. ĐÃ COMMIT + PUSH (`25114f6`).** (1) **README.md viết lại hoàn toàn** theo template README portfolio project cũ của user (PawCare): tiếng Anh, live demo + **video demo YouTube `https://youtu.be/SKMgRnkJIY0`** + cảnh báo cold start 60s + 12 screenshots + features + tech stack + ASCII architecture + getting started (env vars khớp `.env.example`) + project structure + bảng Deployment (link docs/deployment.md) + **Known Issues** (SMTP prod bị Render chặn · Swagger prod 500 · path sai trả 500 · Gemini rate limit). Bản README cũ (nội bộ dev/AI-agent workflow) bỏ. (2) **12 screenshots** user chụp bỏ `docs/screenshots/` (login/applications/application-detail/create-application/jd-insight/cv-match/email-draft/cv-list/cv-detail/analytics/notifications/settings). (3) **Bẫy đã bắt trước khi commit:** link video user dán lần đầu (`Yk7Nlw0r9Po`) đã 404 — verify bằng YouTube oEmbed → video thật là `SKMgRnkJIY0`, sửa cả text lẫn href. (4) **Ghi nhận 5 commit từ session khác 07-22/23 chưa có trong notes:** `7aec100` tối ưu UX sau tạo đơn · `1fa8a60` fix không update appliedDate khi đổi status sang APPLIED · `d4fb60e` mở rộng AI extract JD điền nhiều field hơn · `c3c2d56` tắt mail health indicator prod (Render free chặn outbound SMTP → deploy timeout) · `8cce5d9` đổi `gemini-3.6-flash` + `thinkingLevel: minimal` (2.5-flash 404 với project mới, xem [[reference-gemini-thinking-quota]]), runbook thêm TZ. **CÒN TREO (không chặn, tùy hứng):** ảnh nền login 2MB · Swagger prod 500 · path sai trả 500 thay 404 · toast error Settings.

> **Update 2026-07-22 — TOAST 3 HỆ + CONFIRM MODAL + SCROLL LIST. ĐÃ COMMIT + PUSH (`92e13a7`, `046bd8f`).** (1) **NotificationToaster** (`features/notifications/components/`, mount ở ProtectedLayout): poll unread 30s size 5, mốc `seenMaxId` (ref) chỉ toast thông báo MỚI (tồn đọng lúc mở app → chuông lo; list rỗng lần đầu → mốc 0 để cái đầu tiên vẫn hiện); nhiều cái cùng đợt vào **lần lượt cách 400ms** (setTimeout xếp lịch, CỐ Ý không cleanup — cleanup là hủy oan toast đang xếp hàng); có noti mới → invalidate `REMINDER_KEYS` (fix bug thẻ nhắc nhở không đổi "đã gửi" cho tới khi F5); nền **navy đặc** `bg-primary`. User đã hiểu sâu toàn bộ design qua hỏi-đáp (unread≠mới, toast best-effort chuông đảm bảo, 3 ca sót chấp nhận: tồn đọng / >5 cái-1-nhịp / F5 mất mốc). ĐÃ BÁC: cột DB `displayed` (over-engineer, sai chỗ per-device). (2) **ToastProvider/useToast** (`components/ui/toast.tsx`, bọc App.tsx): toast HÀNH ĐỘNG góc phải DƯỚI, success xanh lá 4s / error đỏ 6s, nền đặc; đã gắn: đổi trạng thái·thêm sự kiện·xóa đơn·gắn CV (detail đơn), tắt/xóa nhắc nhở, lưu/reparse CV (CvDetail), tạo đơn, upload CV. Quy tắc: chỗ có lỗi inline sẵn (statusError, saveError, create form) thì KHÔNG toast error; mutation nền (upload/reparse 202) toast nói "đã gửi" không nói "xong". **Settings CHƯA gắn** (cân nhắc: chỉ đáng toast error). (3) **ConfirmProvider/useConfirm** (`components/ui/confirm-dialog.tsx`): modal xác nhận **promise-based** (cất `resolve` vào ref — deferred pattern), thay 2 chỗ `window.confirm`, gọi `await confirmDialog('...')`; Esc/backdrop = Hủy. ĐÃ CÂN NHẮC inline-modal-per-file theo đề xuất user → giữ context (call site 1 dòng, không bẻ đôi handler). (4) **Scroll**: 3 list trang chi tiết đơn (sự kiện/lịch sử/nhắc nhở) `max-h-60..72 + overflow-y-auto`. (5) **Tạo đơn: Ở LẠI trang + `reset()` form** (user chọn bỏ navigate sang detail; reset chống tạo đơn trùng). (6) `SHOW_MS` toaster user chỉnh 8s→6s. (7) **ReminderJobs user đổi 3' để test toast → ĐÃ TRẢ VỀ 15'** trước commit (3' làm Neon không ngủ được, đốt compute free). **CÒN TREO cũ:** 7B-3 README + screenshots · ảnh nền login 2MB · Swagger prod 500 · path sai trả 500 thay 404.
>
...
- **Phase 7:** Analytics + Polish + Deploy ← **DONE 2026-07-23.** 7A Analytics ✅ (`fd8a7f5`) · 7B-1/7B-2 Polish ✅ (`740054f`) · 7C Deploy ✅ **APP LIVE 2026-07-21** (Vercel + Render + Neon + UptimeRobot) · 7B-3 README + screenshots + video demo ✅ (`25114f6`) → **🎉 TOÀN BỘ PROJECT HOÀN THÀNH**
